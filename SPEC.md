...
5. **Гейт**: для каждого не-optional produces-порта: файл/каталог существует; json → парс + схема; rules — все. Провал → `gate_report.json`; попыток < `gate_retries`+1 → **архивировать попытку** и повторить с добавкой `gate_feedback` (§11); иначе `failed/failed_validation`.
