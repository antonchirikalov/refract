...
dependencies = [
  ...
  "typer>=0.12", "jinja2>=3.1", "ruamel.yaml>=0.18",
]
...
> CHANGED (2026-07-25): добавлена `ruamel.yaml` — для точечных правок из UI (§19.2):
> сменить модель узла или число раундов лупа нельзя, теряя комментарии шаблона, а
> `pyyaml` round-trip не умеет. Всё остальное чтение форматов остаётся на `pyyaml`.
>
...
> **Формальное определение языка — [SPEC-DSL.md](SPEC-DSL.md)** (добавлено 2026-07-27):
> полный синтаксис всех видов нод, каждое поле с дефолтом, грамматика ссылок, система типов
> и таблица ВСЕХ правил валидации с кодами. Этот раздел остаётся нормативным по семантике
> (что движок делает), SPEC-DSL — по языку (что допустимо и что валидатор обязан отвергнуть).
> Расхождение между ними — баг; коды ошибок синхронизируются тестом
> `tests/test_dsl_spec.py`, который также валидирует сводный пример из §13 того документа.
...
> CHANGED (2026-07-25): у пайплайна появилось необязательное поле
> `input_mode: documents | brief` (default `documents`). Исполнение от него НЕ зависит
> — в обоих случаях это просто файлы в `input/`; поле говорит клиенту, что спрашивать
> у пользователя: папку с документами или текст брифа/темы (ресерч-сценарий,
> SPEC-UI §5). Выводить это из комментариев ненадёжно, поэтому поле типизировано.
...
> CHANGED (2026-07-26): добавлен код `E_MCP_UNDECLARED` — агент просит
> `mcp:<server>`, которого нет в `~/.refract/mcp.yaml`. Раньше это не проверялось
> вообще: шаг запускался без инструмента, под который агент написан, и падал (или молча
> не пользовался им) уже в рантайме. `ValidationContext.known_mcp_servers = None`
> означает «MCP-конфиг не загружен» и проверку выключает — так делают инструменты,
> которым он неизвестен; CLI и API всегда передают реальный набор.
...
> CHANGED (2026-07-27): **транзиентная ошибка провайдера — тоже инфра-ошибка**. Адаптер
> (§12) классифицирует ответ провайдера: `429`, `5xx` и `401` от лимитера возвращаются как
> `completed=False` и проходят через backoff-ретраи; исчерпанная квота (`403`/`401` с
> billing-сообщением) и клиентские ошибки (`400`, `404` — плохой запрос, неизвестная
> модель) остаются терминальными `failed_agent`, потому что повтор их не лечит. Причина:
> боевой прогон потерял ОБА шага `map_over` из-за одного `401`, который лимитер отдаёт под
> нагрузкой — тот же ключ через минуту отвечал `200`, и `resume --retry-failed` восстановил
> шаги без изменений. Вместе с этим `agent_error` от адаптера теперь доезжает до леджера и
> при исчерпании инфра-ретраев: строка «infra retries exhausted» без причины отправляла
> читателя искать баг в движке, когда провайдер всё это время отвечал `429`.
...
>
> CHANGED (2026-07-27): ещё два правила по итогам второго боевого прогона. У
> `requirements@v1` regex заголовка привязан к НАЧАЛУ файла (`\A#\s*Requirements:`):
> писатель самовольно добавлял YAML front matter с придуманной метаинформацией
> (`fr_count: 9` при десяти FR), и критику приходилось спорить о ней вместо требований.
> У `design_doc@v1` добавлено наличие секции про риски (`^#{2,3}[^\n]*[Rr]isk`): оба
> кандидата дизайна вышли вообще без раздела рисков, и раунд петли ушёл на структурное
> замечание, проверяемое механически. Формулировка заголовка остаётся свободной — гейт
> проверяет только присутствие.
...
> CHANGED (2026-07-27): `body` может быть ЦЕПОЧКОЙ элементов (`body: [ {agent: …}, … ]`).
> Элементы исполняются по порядку внутри раунда, каждый видит выход предыдущего по ссылке
> `@prev` (или `@prev.<port>`), а **выход последнего элемента и есть черновик раунда**:
> именно его судит критик (`@body`), из него собирается выход ноды, и из него берётся
> `_previous` для следующего раунда. Ревизия (`_previous` + `_verdict`) достаётся ТОЛЬКО
> первому элементу — он переписывает, остальные обрабатывают то, что получили.
> Идентификаторы шагов: цепочка из одного элемента сохраняет исторические `body:r{n}` /
> `body_r{n}`, цепочка длиннее — `body1:r{n}`, `body2:r{n}` и т. д. Причина: роли внутри
> петли часто больше двух («написать → проверить факты → отсудить»), тогда как
> КОНТРОЛЬНОЕ решение остаётся одним вердиктом. Контроллер поэтому намеренно один:
> контейнер — это архетип «элементы + один контроллер + один контрольный тип», а не
> свободный комбинатор. Свободная композиция с условием выхода в YAML отклонена: она
> требует языка выражений над артефактами, ослабляет статическую валидацию и открывает
> обход I4. `@body`/`@prev` вне петли и `@prev` в первом элементе → `E_LOOP_SHAPE`
> (раньше такая ссылка молча игнорировалась, и агент работал без артефакта).
...
отклонены там же). **Фаза 5**: `discover` — сетевой источник коллекции для
ресерч-пайплайнов, механика в §20 (был DESIGN NOTE ниже).
...
>
> CHANGED (2026-07-25, уточнение): отмена касается ОБЩЕГО словаря операций. Две
> точечные правки, на которые есть прямой спрос от UI-инспектора — модель узла (или
> его под-блока) и параметры вида `max_rounds`/`workers` — сделаны отдельной
> типизированной ручкой (§19.2.1). Это не словарь на все случаи, а два сеттера с
> валидацией результата; YAML при этом сохраняет комментарии (round-trip), потому что
> пайплайн проекта заводится из шаблона с пояснениями.
...
#### 19.2.1. Точечные правки узла (для UI-инспектора)

```
PATCH /api/projects/{id}/pipelines/{name}/nodes/{node_id}
      {model?: str|null, block?: body|critic|selector, params?: {<key>: <scalar>}}
```

- `model` пишется в `params.model` ноды либо в `model` под-блока (`block`); `null`
  снимает поле — узел вернётся к `defaults.model` проекта.
- `params` — только скалярные ключи существующей params-модели узла (§8.2);
  неизвестный ключ или неверный тип → `422` с ошибкой pydantic.
- Дальше как §19.2: применили → полная валидация → атомарная запись, при блокирующих
  ошибках НЕ пишем и отвечаем `409` с отчётом; активный ран → `409`; ответ отдаёт
  свежий `hash`. Комментарии и порядок ключей файла сохраняются.

Это единственный способ записи из UI v0.1; полная замена файла (`PUT`) остаётся для
CLI и внешних инструментов.
...
## 20. `discover` — сетевой источник коллекции (фаза 5)

> CHANGED (2026-07-25): раздел добавлен. §17 содержал DESIGN NOTE о `discover` с тремя
> открытыми вопросами и пометкой «отдельная фаза»; здесь они закрыты, потому что
> ресерч-сценарий UI (дал тему → сами нашли источники) без этой ноды не существует.

**Зачем.** Ресерч-пайплайн: пользователь даёт тему, а источники ищутся в сети. Обычный
агент коллекцию производить не может (I6), а `builtin/scanner` — детерминированный
builtin над файлами `input/` и в сеть не ходит. `discover` — ВТОРОЙ (и последний в v0.1)
легальный производитель `collection<source@v1>` «из ничего». Низ графа не меняется:
`discover` встаёт на место `scan`, дальше те же map → loop → select.

### 20.1. Форма ноды

```yaml
  - id: brief
    type: builtin/brief            # читает input/, отдаёт бриф как артефакт

  - id: find
    type: discover
    agent: source_finder@1         # backed рантаймом: needs [..., mcp:tavily-remote]
    inputs: { brief: brief.brief }
    params: { min_sources: 3, workers: 1 }
```

Выход ноды всегда один порт `sources: collection<source@v1>` (имя как у scanner).
Агент при этом производит РОВНО ОДИН артефакт — каталог `found_sources@v1`
(`kind: dir`), по файлу на найденный источник плюс необязательный `_index.json`
(`[{file, title, url}]`). Коллекцию из этого каталога собирает ДВИЖОК — поэтому I6
не нарушается: агент не производит коллекцию, он производит один каталог.

Валидатор (`E_DISCOVER_SHAPE`): агент discover-ноды обязан иметь ровно один
не-optional produces-порт типа с `kind: dir`; consumes-порты разрешены любые и
подаются через `inputs` как обычно.

### 20.2. Исполнение

Шаг discover — обычный agent-шаг §10.2 целиком (промпт из контракта I5, гейт,
gate-ретраи, attempts, трасса I9). После успеха движок собирает коллекцию:

- элемент на каждый файл/подкаталог верхнего уровня в каталоге агента, в
  отсортированном порядке (dot-элементы и `_index.json` пропускаются);
- `slug` = `unique_slug(slugify(имя))`, `source_hash` = sha256 содержимого —
  ТЕ ЖЕ правила, что у scanner (§13), никакой новой схемы хеширования (закрывает
  открытый вопрос о детерминизме слагов сетевых находок);
- `source` = имя файла, `origin` (url из `_index.json`, если есть) — в поле `error`
  не пишется, попадает в `_index.json` копией рядом с манифестом;
- сборка в `steps/<id>/_out/sources/` — идемпотентна при resume, как у map;
- `ok < min_sources` (default 1) → нода `failed` (`error="no sources discovered"`).

### 20.3. Reuse (закрывает открытый вопрос)

`discover` переиспользуется по правилам ОБЫЧНОГО agent-шага: не переисполняется, если
не изменились его входы (хэш брифа) и пакет агента, и нода не форсирована. Причина:
сеть недетерминирована, и «всегда переисполнять» инвалидировало бы весь низ графа на
каждом прогоне — прямо против смысла reuse. Свежий поиск — явное действие:
`refract rerun --from <discover>` или `--force`. Низ графа при этом всё равно диффится
по `source_hash`: если свежий поиск нашёл те же документы, элементы map переиспользуются.

### 20.4. `builtin/brief` и типы

`builtin/brief`: `produces: [{port: brief, type: brief@v1}]`. Params: `file` (имя файла
в input-папке; default `brief.md`), `input` (оверрайд пути). Читает файл, копирует в
выход; отсутствует или пуст → нода `failed`. Детерминирован, без раннера — как scanner.

Новые типы библиотеки: `brief@v1` (`kind: file, format: markdown`), `found_sources@v1`
(`kind: dir`).

`input_mode: brief` у пайплайна (§8) говорит UI спрашивать текст темы вместо папки:
текст сохраняется как `input/brief.md`, дальше его читает `builtin/brief`.

### 20.5. Обязательные тесты

- `test_discover`: fan-out (каталог агента из 3 файлов → коллекция из 3 элементов →
  map по ним); слаги и `source_hash` детерминированы и совпадают с правилами scanner;
  пустой каталог агента → гейт-провал; `ok < min_sources` → нода failed; идемпотентная
  пересборка при resume; reuse при неизменном брифе и переисполнение при изменённом.
- `test_graph_validation`: `E_DISCOVER_SHAPE` (агент без dir-порта / с двумя портами);
  `map:` по выходу discover разрешён (не `E_NESTED_MAP`).
- `test_builtin_brief`: файл прочитан; нет файла → failed; пустой файл → failed.

## 21. Чекпойнты — парковка рана на выверку человеком (фаза 5)

> CHANGED (2026-07-25): раздел добавлен. Причина — редакторский сценарий: «дошли до
> требований, я их выверил, дальше работай». Из-за отсутствия этого механизма
> пайплайны разрезаны по продуктам (extract / solution_design), и общий префикс
> `scan → extract → refine` дублируется, а reuse и происхождение между проектами
> теряются. Механизм переиспользует уже существующую паузу `waiting_human` (§16.9/§16.10).
...
version: "0.1"
name: full
checkpoints: [refine]        # ← id узлов, ПОСЛЕ которых ран парковать
nodes: [...]
```

Поле уровня пайплайна, а не параметр ноды: чекпойнт нужен всем видам нод (включая
builtin), а params у них разные; плюс единый список читается и рендерится как есть.
Неизвестный id → `E_UNKNOWN_NODE_REF`.

Разовое переопределение на запуск (объявление в файле не требуется):
`POST /api/projects/{id}/runs {stop_after: ["refine"]}` и
`refract run <project> --stop-after refine`. Набор рана = `checkpoints ∪ stop_after`;
он фиксируется в снапшоте, как и всё остальное (§9).

### 21.2. Механика

Чекпойнт срабатывает ПОСЛЕ того, как нода дошла до `done`/`reused` и её выходы
собраны — то есть артефакт уже есть и его можно смотреть:

1. Движок пишет `steps/<node>/checkpoint/request.json`
   (`{node, outputs: [<путь артефакта>...]}`), ставит рану статус `waiting_human`,
   пишет `awaiting_checkpoint: <node>` в леджер и эмитит событие `question`.
2. Новые ноды не стартуют. Уже исполняющиеся доводятся до конца (как при cancel,
   §9) — их результаты в леджере, ничего не теряется.
3. Человек смотрит выход и при желании ПРАВИТ его прямо в `steps/<node>/_out/…`
   (или `main/output/…`). Это единственное легальное исключение из I2: узел уже
   `done`, и ручная правка — осознанное действие человека, а не движка.
4. `refract answer <run_dir> <node> continue` (или `/answers`) пишет
   `checkpoint/decision.json` `{approved: true|false, answer}`. `approved: false` →
   ран `cancelled`.
5. `resume` продолжает низ графа. Правка из п.3 подхватывается как есть: низ читает
   выход узла, а не свою копию.

Повторное прохождение того же чекпойнта в одном ране не парковывает: наличие
`decision.json` с `approved: true` — сигнал «этот чекпойнт пройден».

### 21.3. Отображение

`refract status` печатает `awaiting checkpoint: <node>` и пути выходов. UI показывает
баннер с ссылками на артефакты и кнопкой «Продолжить» (`resume` после ответа).

### 21.4. Обязательные тесты

- `test_checkpoint`: ран парковывается после объявленного узла (`waiting_human`,
  `awaiting_checkpoint`, `request.json` с путями выходов); низ графа НЕ исполнялся;
  `resume` без решения снова парковывает; `continue` → низ доезжает; `reject` →
  `cancelled`; правка выхода между парковкой и resume видна низу графа; `--stop-after`
  работает без объявления в файле; неизвестный id → `E_UNKNOWN_NODE_REF`.
