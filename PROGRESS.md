...
| 1 | metanodes (loop/select), map_over.models, winner_model binding, rerun/reuse + `refract rerun` **done** (test_loop/test_select/test_map_over/test_reuse green, spec-audited); 3 library templates, OpencodeRuntime execution (serve-per-step, I1/heartbeats/auto-approve/kill) + docs/opencode-smoke.md **done** (verified vs real opencode 1.18.4 to the LLM boundary); all spectra agents migrated (arch_probe/critic, effort_estimator, illustrator, confluence_publisher, word_form_builder) **done**. Phase 1 engine + content complete; live-LLM smoke run PASSED (demo pipeline on real opencode 1.18.4 + openai: completed, gate-retry recovered a bad output, I8 secrets clean, no leaked processes) | done |
