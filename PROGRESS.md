...
| 1 | metanodes (loop/select), map_over.models, winner_model binding, rerun/reuse + `refract rerun` **done** (test_loop/test_select/test_map_over/test_reuse green, spec-audited); 3 library templates, OpencodeRuntime execution (serve-per-step, I1/heartbeats/auto-approve/kill) + docs/opencode-smoke.md **done** (adapter written, not automated-tested per §18 — needs a manual smoke run on real opencode); remaining: remaining spectra agents (arch/effort/illustrator/publisher/word_form) | in progress |
