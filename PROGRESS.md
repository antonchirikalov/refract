...
| 2 | REST/WS API (§15): projects/pipelines/validate/runs/artifacts/cancel/resume/models/fs + WS events **done** (pause/answers deferred to §16.9/phase 3); frontend UI spec separate | in progress |
| 3 | HITL (question@v1 → waiting_human → `refract answer` / API answers → resume) **done** for plain agent nodes (guarded inside map/loop); capability tiers + confirmations REMAIN (SPEC names them but doesn't specify mechanics — needs a design) | in progress |
