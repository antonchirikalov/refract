...
Универсальный движок исполнения конвейеров LLM-агентов: вы описываете работу одним файлом,
движок её выполняет и следит, чтобы каждый шаг отдал ровно тот результат, который обещал.
...
Рис. 1. Название буквальное: один запрос преломляется в цепочку агентов, каждый из которых
отвечает за свою часть работы и передаёт дальше документ, а не разговор.

### Зачем он нужен

refract — про подготовку качественных выверенных документов без выдумок. Не ответ в чате, а
документ, который не стыдно отдать заказчику или команде: каждая цифра прослеживается до
источника, спорные места стоят открытыми вопросами, а не замазаны, предложения помечены как
предложения, а не выданы за установленный факт.

Одним запросом такой документ не получается, поэтому работа разбита на шаги, и на каждом
конкретный агент отвечает за свою часть: один вытаскивает факты из источника, другой сводит их
воедино, третий сверяет написанное с источниками, четвёртый судит результат и отправляет на
доработку, пока документ не годен. Плюс человек в середине — там, где решать должен он.

Какие документы так готовят:

- документ требований по запросу на предложение (RFP) и сопутствующим материалам —
  коммерческому предложению, транскриптам звонков, переписке. Противоречия между источниками
  не проглатываются: расхождение попадает в документ как конфликт с указанием, чем он
  разрешён, или как открытый вопрос;
- технический дизайн: проект решения по требованиям — подход, архитектура, технологические
  решения с обоснованием, риски, допущения к подтверждению. Проект можно поручить сразу
  нескольким моделям и выбрать сильнейший вариант;
- исследование темы: на входе не документы, а вопрос; агент сам ищет источники в сети,
  скачивает их, дальше идёт та же цепочка сведения и проверки;
- вопросы к архитектуре для установочной встречи: сначала формулируются по требованиям, затем
  отбираются и упорядочиваются в список, пригодный для обсуждения;
- оформление и публикация готового документа: иллюстрации, выкладка в корпоративную вики.

Движок не привязан ни к одной из этих задач. Он знает только три вещи: шаги, файлы между шагами
и типы этих файлов. Что именно делает конвейер, задают описание и набор агентов — поэтому новая
задача добавляется файлом описания и папкой агента, а не правкой движка.
...
Рис. 2. Три зоны: что вы объявляете, что делает движок, где выполняется работа. Обратная
...
- Управление только из структурированных файлов. Продолжить петлю, выбрать победителя,
  ...
  тексте. Уговорить конвейер словами нельзя.
- Независимость от провайдера и модели. Модель задаётся строкой на каждом шаге: любой
  ...
  Дешёвая модель на извлечении, дорогая на проектировании — вопрос одной строки.
...
- Параллельность с ограничениями. Независимые шаги идут одновременно, обработка набора
  ...
  ограничены — конкурс моделей не превращается в шторм запросов.
...
refract исполняет описания конвейеров и больше ничего не навязывает. У него нет своей базы
данных, своего формата хранения результатов, своего интерфейса для конечного пользователя.
Проект — папка на диске с описанием, входными данными и прогонами; всё, что движок производит,
остаётся файлами.
...
Не платформа для чат-приложений. Нет диалогового интерфейса для конечного пользователя и нет
публикации конвейера как приложения: результат работы — файлы, а не разговор.
...
Не визуальный конструктор конвейеров. Веб-интерфейс показывает граф и прогон, а из правок умеет
выбор модели и число раундов петли. Описания пишутся руками, как код.
...
Рис. 3. Тот же конвейер как граф. На связях — типы артефактов; дуга снизу показывает, что шаг
...
Рис. 4. Тот же конвейер в интерфейсе. Петля и выбор нарисованы контейнерами с элементами
...
Рис. 5. Щелчок по элементу открывает панель свойств: роль элемента и выбор модели. Правка
...
Рис. 6. Прогон дошёл до указанного шага, собрал его результат и ждёт человека. Документ можно
