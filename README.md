...
Универсальный движок исполнения конвейеров LLM-агентов: конвейер описывается декларативно,
движок его валидирует и исполняет, контролируя соответствие результата каждого шага контракту.
...
Рис. 1. Название отражает принцип работы: исходный запрос преломляется в цепочку агентов, где
каждый отвечает за отдельную часть работы, а между шагами передаётся документ, а не диалог.

### Цель проекта

Подготовка качественных документов в задачах предпроектного и проектного анализа: требования
по запросу на предложение (RFP), технические дизайны, исследования и подобные. Целевое
свойство результата — прослеживаемость: каждое утверждение возводится к источнику,
неразрешённые расхождения фиксируются как открытые вопросы, проектные предложения отделены от
установленных фактов.

Одиночный запрос к модели этого свойства не обеспечивает, поэтому работа декомпозируется на
шаги с разделением ответственности: извлечение фактов из отдельного источника, сведение
извлечённого в один документ, сверка написанного с источниками, критическая оценка с решением
о доработке, выверка человеком в заданных точках.
