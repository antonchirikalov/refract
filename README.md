...
![Что подаётся движку и что он с этим делает: описание, агенты и типы на входе; проверка, планирование, исполнение шага и проверка результата внутри; opencode и модель на исполнении; папка прогона на выходе](assets/engine-overview.png)
...
и его ключ на месте, и записывает в конфигурацию шага. Дальше он к провайдеру не обращается: на
шаг поднимается opencode отдельным процессом, получает эту конфигурацию — какую модель
использовать, какие инструменты разрешены, где лежат файлы, — и уже сам ходит по сети,
разговаривает с провайдером и вызывает инструменты, а движок общается с ним по локальному REST.

Разделение ролей такое: назначить модель и проверить, что она доступна, — дело описания и движка;
поговорить с провайдером — дело opencode. Отсюда две выгоды: движку не нужно знать про протоколы
...
493 теста на подставном рантайме — без сети, без ключей, без opencode; 18 сквозных тестов
...
теста, 18 сквозных, строгая типизация, линтер.
...
Подробный журнал прогонов и найденных дефектов ведётся внутренне.
