...
Есть класс задач, где от языковой модели нужен не ответ в чате, а рабочий документ, который не
стыдно отдать заказчику или команде. Одним запросом такой документ не получить: его надо
собрать из источников, свести противоречия, проверить, отдать человеку на выверку, доработать
по замечаниям. Именно эту работу исполняет refract.

Что на нём собирают:

- обработка запроса на предложение (RFP) и сопутствующих материалов — коммерческое предложение,
  транскрипты звонков, переписка: на выходе документ требований, где каждая цифра
  прослеживается до источника, а спорные места помечены как открытые вопросы;
- технический дизайн: по требованиям делается проект решения — подход, архитектура,
  технологические решения с обоснованием, риски, допущения к подтверждению; проект можно
  поручить сразу нескольким моделям и выбрать сильнейший вариант;
...
- подготовка к установочной встрече: по требованиям формируются вопросы к архитектуре, затем
  отбираются и упорядочиваются в пригодный для обсуждения список;
- оформление и публикация результата: иллюстрации к документу, выкладка в корпоративную вики.
