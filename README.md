...
- Тип как переиспользуемый контракт. Структурированный вывод по схеме есть у обоих, но в Dify
  ...
  многими агентами и участвует в проверке связей: соединить выход одного типа со входом другого
  нельзя, это ошибка до запуска.
...
- выбор модели для каждого шага и подключение любых провайдеров, включая совместимые с OpenAI по
  базовому адресу;
...
| Документ | О чём |
|---|---|
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | технический разбор: принципы, исполнение, состояние, восстановление после сбоя |
...
Нормативные спецификации — семантики движка, языка описания и веб-интерфейса — ведутся
внутренне и в репозиторий не входят. Ссылки вида «SPEC §N» в коде и комментариях указывают
на их разделы; расхождение кода и спецификации считается дефектом и правится явно.
